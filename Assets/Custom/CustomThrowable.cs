...
namespace Valve.VR.InteractionSystem
{
    ...
    public class CustomThrowable : MonoBehaviour
    {
        ...
        private float scaleReleaseVelocity = 0f;

        [Tooltip("When detaching the object, should it return to its original parent?")]
        public bool restoreOriginalParent = false;

        public bool enableTwoHandScaling = true;
        private Hand _firstAttachedHand;
        private Hand _secondAttachedHand;
        [HideInInspector]
        public bool twoHandsAttached = false;
        ...
        {
            if (enableTwoHandScaling && twoHandsAttached)
            {
                var newDirection = _firstAttachedHand.transform.position -
                    _secondAttachedHand.transform.position;

                if (newDirection.x == initialDirection.x && newDirection.y == initialDirection.y && newDirection.z == initialDirection.z)
                    return;

                var rotation = new Quaternion();
                rotation.SetFromToRotation(initialDirection, newDirection);
                //gameObject.transform.rotation = rotation;

                var newDistance = newDirection.sqrMagnitude;
                gameObject.transform.localScale *= newDistance /  initialHandsDistance;
        ...
        protected virtual void OnAttachedToHand(Hand hand)
        {
            ...
            {
                if (_firstAttachedHand != null)
                {
                    _secondAttachedHand = hand;
                    twoHandsAttached = true;

                    initialDirection = _firstAttachedHand.transform.position -
                    _secondAttachedHand.transform.position;
                    initialHandsDistance = initialDirection.sqrMagnitude;
                }
                else
                {
                    _firstAttachedHand = hand;
                    twoHandsAttached = false;
        ...
        protected virtual void OnDetachedFromHand(Hand hand)
        {
            ...
            {
                twoHandsAttached = false;
                if (_secondAttachedHand != null)
                {
                    if (hand == _secondAttachedHand)
                        _secondAttachedHand = null;
                    else
                    {
                        var grabtype = _firstAttachedHand.currentAttachedObjectInfo.Value.grabbedWithType;
                        ...
                        _firstAttachedHand = _secondAttachedHand;
                        _secondAttachedHand = null;
                        _firstAttachedHand.AttachObject(gameObject,grabtype,attachmentFlags);
                    }
                        
                }
                else
                {
                    _firstAttachedHand = null;
                    attached = false;
